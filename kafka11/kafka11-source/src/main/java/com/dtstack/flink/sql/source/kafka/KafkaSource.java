...
public class KafkaSource implements IStreamSourceGener<Table> {
	...
	public Table genStreamSource(SourceTableInfo sourceTableInfo, StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
		...
		Boolean topicIsPattern = kafka011SourceTableInfo.getKafkaBooleanParam("topicIsPattern");
        ...
        if("json".equalsIgnoreCase(kafka011SourceTableInfo.getSourceDataType())){
			...
			if (topicIsPattern){
				kafkaSrc = new CustomerKafka011Consumer(topicName, new CustomerCommonDeserialization(),props);
			}else {
				kafkaSrc = new CustomerKafka011Consumer(Pattern.compile(topicName), new CustomerCommonDeserialization(),props);
			}

...
public class KafkaSource implements IStreamSourceGener<Table> {
	...
	public Table genStreamSource(SourceTableInfo sourceTableInfo, StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
		...
		Boolean topicIsPattern = kafka09SourceTableInfo.getKafkaBooleanParam("topicIsPattern");
		...
		if("json".equalsIgnoreCase(kafka09SourceTableInfo.getSourceDataType())){
			...
			if (topicIsPattern){
				kafkaSrc = new CustomerKafka09Consumer(topicName, new CustomerCommonDeserialization(),props);
			}else {
				kafkaSrc = new CustomerKafka09Consumer(Pattern.compile(topicName), new CustomerCommonDeserialization(),props);
			}
